...
import com.springJourney.lazy.config.EagerAppConfig;
...
@SpringBootTest
@ContextConfiguration(classes = EagerAppConfig.class)
public class EagerLoadingTest {

    @Test
    public void testEagerLoading(@Autowired ApplicationContext applicationContext) {
        System.out.println("[" + System.currentTimeMillis() + "] Spring 容器已启动（无 @Lazy 注解）");

        // 验证 Bean 已创建 此处断点，启动时就会调用LazySingletonService的构造方法
